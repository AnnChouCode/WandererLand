<template>
  <div class="py-7 py-md-9">
    ...
    <div class="position-relative">
      ...
      <SwiperSlide v-for="(item, idx) in currentDatas" :key="idx">
        <router-link :to="`${DataInfo[dataCategory].infoPath}/${item.id}`">
          <div
            class="position-relative d-flex justify-content-center align-items-center ratio-1x1 overflow-hidden product-img" :class="{ 'rounded-circle': isArtistBlock }">
        ...
        <div class="py-3 px-0 px-md-4">
          <router-link :to="`${DataInfo[dataCategory].infoPath}/${item.id}`">
            <h3 class="mb-2 fs-info fs-md-5 fw-bold text-default" :class="{ 'text-center': isArtistBlock }">{{ item.title }}</h3>
          </router-link>
    ...
    <div class="position-absolute end-0 z-1" style="top:210px">
      <i class="bi bi-arrow-right-circle text-warning fs-1"></i>
    </div>
    <div class="position-absolute z-1" style="top:210px">
      <i class="position-absolute bi bi-arrow-left-circle text-warning fs-1"></i>
    </div>
    </div>
  </div>
  <!-- <div class="container py-7 py-md-9">
  <h2 class="mb-7 mb-md-8 fs-2 fs-md-1">{{ adTitle[adCategoryTitle] }}</h2>
  ...
  <Swiper
    ...
    >
      ...
      <router-link :to="`/Datainfo/${item.id}`">
