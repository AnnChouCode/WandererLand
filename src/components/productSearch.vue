<template>
    <div class="position-relative">
        ...
        <ul v-if="search" class="position-absolute z-1 start-0 mt-1 list-unstyled overflow-y-scroll bg-white border border-primary w-100 dropdown-scrollbar">
            <li v-if="!filterProducts.length" class="p-3 text-info">
              未搜尋到符合輸入的作品
            </li>
            <li v-else v-for="item in filterProducts" :key="item.id">
              <router-link :to="`/productinfo/${item.id}`" class="d-block p-3 text-default link-hover-bg">{{ item.title }}</router-link>
            </li>
        </ul>
    </div>
</template>
